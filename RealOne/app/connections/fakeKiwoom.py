...
from app.services.slackService import SlackService
from app.connections.realKiwoom import *            # 로그인을 위한 클래스

# class KiwoomSingle(QWidget, metaclass=Singleton):       # QMainWindow : PyQt5에서 윈도우 생성시 필요한 함수
...
class KiwoomYo:
    ...
    def _set_signals_slots(self):
        self.kiwooms.kiwoom.OnEventConnect.connect(self.OnEventConnect)
        # self.kiwooms.kiwoom.OnReceiveTrData.connect(self.trdata_slot)
        # self.kiwooms.kiwoom.OnReceiveRealData.connect(self.OnReceiveRealData)
        # self.kiwooms.kiwoom.OnReceiveMsg.connect(self.OnReceiveMsg)
        # self.kiwooms.kiwoom.OnReceiveChejanData.connect(self.OnReceiveChejanData)

        # self.kiwooms.kiwoom.OnReceiveRealCondition.connect(self.OnReceiveRealCondition)
        # self.kiwooms.kiwoom.OnReceiveTrCondition.connect(self.OnReceiveTrCondition)
        # self.kiwooms.kiwoom.OnReceiveConditionVer.connect(self.OnReceiveConditionVer)
    ...
    def CommConnect(self, block=True):
        ...
        :return: None
        """
        self.kiwooms.kiwoom.dynamicCall("CommConnect()")
    ...
    def CommRqData(self, rqname, trcode, next, screen):
        ...
        """
        msg ="CommRqData"
        print("스크린: %s, 요청이름: %s, tr코드: %s --- %s" %(screen, screen, trcode, msg))
 
        # print("CommRqData")
        self.kiwooms.kiwoom.dynamicCall("CommRqData(String, String, int, String)", rqname, trcode, next, screen)
    ...
    def GetLoginInfo(self, tag):
        ...
        """
        data = self.kiwooms.kiwoom.dynamicCall("GetLoginInfo(QString)", tag)
    ...
    def SendOrder(self, rqname, screen, accno, order_type, code, quantity, price, hoga, order_no):
        ...
        :return:
        """
        ret = self.kiwooms.kiwoom.dynamicCall("SendOrder(QString, QString, QString, int, QString, int, int, QString, QString)",
    ...
    def SetInputValue(self, id, value):
        ...
        """
        print("SetInputValue")
        self.kiwooms.kiwoom.dynamicCall("SetInputValue(String, String)", id, value)
    ...
    def DisconnectRealData(self, screen):
        ...
        :return: None
        """
        self.kiwooms.kiwoom.dynamicCall("DisconnectRealData(QString)", screen)
    ...
    def GetRepeatCnt(self, trcode, rqname):
        ...
        :return: 멀티데이터의 행의 개수
        """
        count = self.kiwooms.kiwoom.dynamicCall("GetRepeatCnt(QString, QString)", trcode, rqname)
        return count
    ...
    def CommKwRqData(self, arr_code, next, code_count, type, rqname, screen):
        ...
        :return:
        """
        ret = self.kiwooms.kiwoom.dynamicCall("CommKwRqData(QString, bool, int, int, QString, QString)", arr_code, next, code_count, type, rqname, screen);
        return ret
    ...
    def GetAPIModulePath(self):
        ...
        :return: 모듈의 경로
        """
        ret = self.kiwooms.kiwoom.dynamicCall("GetAPIModulePath()")
        return ret
    ...
    def GetCodeListByMarket(self, market):
        ...
        """
        data = self.kiwooms.kiwoom.dynamicCall("GetCodeListByMarket(QString)", market)
        tokens = data.split(';')[:-1]
        return tokens
    ...
    def GetConnectState(self):
        ...
        :return: 0:미연결, 1: 연결완료
        """
        ret = self.kiwooms.kiwoom.dynamicCall("GetConnectState()")
        return ret
    ...
    def GetMasterCodeName(self, code):
        ...
        :return: 종목명
        """
        data = self.kiwooms.kiwoom.dynamicCall("GetMasterCodeName(QString)", code)
        return data
    ...
    def GetMasterListedStockCnt(self, code):
        ...
        :return: 상장주식수
        """
        data = self.kiwooms.kiwoom.dynamicCall("GetMasterListedStockCnt(QString)", code)
        return data
    ...
    def GetMasterConstruction(self, code):
        ...
        """
        data = self.kiwooms.kiwoom.dynamicCall("GetMasterConstruction(QString)", code)
        return data
    ...
    def GetMasterListedStockDate(self, code):
        ...
        """
        data = self.kiwooms.kiwoom.dynamicCall("GetMasterListedStockDate(QString)", code)
    ...
    def GetMasterLastPrice(self, code):
        ...
        :return: 전일가
        """
        data = self.kiwooms.kiwoom.dynamicCall("GetMasterLastPrice(QString)", code)
        return int(data)
    ...
    def GetMasterStockState(self, code):
        ...
        :return: 종목상태
        """
        data = self.kiwooms.kiwoom.dynamicCall("GetMasterStockState(QString)", code)
        return data.split("|")

    def GetDataCount(self, record):
        print("GetDataCount")
        count = self.kiwooms.kiwoom.dynamicCall("GetDataCount(QString)", record)
        return count

    def GetOutputValue(self, record, repeat_index, item_index):
        print("GetOutputValue")
        count = self.kiwooms.kiwoom.dynamicCall("GetOutputValue(QString, int, int)", record, repeat_index, item_index)
        return count
    ...
    def GetCommData(self, trcode, rqname, index, item):
        ...
        data = self.kiwooms.kiwoom.dynamicCall("GetCommData(QString, QString, int, QString)", trcode, rqname, index, item)
        return data.strip()

    def GetCommRealData(self, code, fid):
        print("GetCommRealData")
        data = self.kiwooms.kiwoom.dynamicCall("GetCommRealData(QString, int)", code, fid)
        return data

    def GetChejanData(self, fid):
        print("GetChejanData")
        data = self.kiwooms.kiwoom.dynamicCall("GetChejanData(int)", fid)
        return data

    def GetThemeGroupList(self, type=1):
        print("GetChejanData")
        data = self.kiwooms.kiwoom.dynamicCall("GetThemeGroupList(int)", type)
        tokens = data.split(';')
        if type == 0:
    ...
    def GetThemeGroupCode(self, theme_code):
        ...
        data = self.kiwooms.kiwoom.dynamicCall("GetThemeGroupCode(QString)", theme_code)
        data = data.split(';')
        return [x[1:] for x in data]

    def GetFutureList(self):
        print("GetFutureList")
        data = self.kiwooms.kiwoom.dynamicCall("GetFutureList()")
        return data
    ...
    def SetRealReg(self, screen, code_list, fid_list, opt_type):
        print("SetRealReg")
        ret = self.kiwooms.kiwoom.dynamicCall("SetRealReg(QString, QString, QString, QString)", screen, code_list, fid_list, opt_type)
        return ret

    def SetRealRemove(self, screen, del_code):
        print("SetRealRemove")
        ret = self.kiwooms.kiwoom.dynamicCall("SetRealRemove(QString, QString)", screen, del_code)
        return ret
    ...
    def GetConditionLoad(self, block=True):
        ...
        self.kiwooms.kiwoom.dynamicCall("GetConditionLoad()")
        if block:
    ...
    def GetConditionNameList(self):
        ...
        data = self.kiwooms.kiwoom.dynamicCall("GetConditionNameList()")
    ...
    def SendCondition(self, screen, cond_name, cond_index, search, block=True):
        ...
        self.kiwooms.kiwoom.dynamicCall("SendCondition(QString, QString, int, int)", screen, cond_name, cond_index, search)
    ...
    def SendConditionStop(self, screen, cond_name, index):
        ...
        self.kiwooms.kiwoom.dynamicCall("SendConditionStop(QString, QString, int)", screen, cond_name, index)

    def GetCommDataEx(self, trcode, rqname):
        print("GetCommDataEx")
        data = self.kiwooms.kiwoom.dynamicCall("GetCommDataEx(QString, QString)", trcode, rqname)
        return data
